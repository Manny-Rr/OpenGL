...
int main() {
   ...
   // glfwSetKeyCallback(window_one, process_input);
    ...
    while (!glfwWindowShouldClose(window_one)) {
        ...
        main_cube_shader.set_vec3("point_light.diffuse", glm::vec3(0.0f, 0.0f, 1.0f));
        main_cube_shader.set_vec3("point_light.specular", glm::vec3(0.0f, 0.0f, 1.0f));


        //multiple lights
        for (int i = 0; i < 4; i++) {
            std::string number = std::to_string(i);
            main_cube_shader.set_vec3 ( ("multi_lights[" + number + "].position") .c_str(), pointLightPositions[i]);
            main_cube_shader.set_float( ("multi_lights[" + number + "].constant") .c_str(), 1.0f);
            main_cube_shader.set_float( ("multi_lights[" + number + "].linear")   .c_str(), 0.07f);
            main_cube_shader.set_float( ("multi_lights[" + number + "].quadratic").c_str(), 0.017f);
            main_cube_shader.set_vec3 ( ("multi_lights[" + number + "].ambient")  .c_str(), glm::vec3(0.0f));
            main_cube_shader.set_vec3 ( ("multi_lights[" + number + "].diffuse")  .c_str(), glm::vec3(0.0f,0.0f,1.0f));
            main_cube_shader.set_vec3 ( ("multi_lights[" + number + "].specular") .c_str(), glm::vec3(1.0f));
        }


        //spot light
        main_cube_shader.set_vec3("spot_light.light_front", camera.camera_direction);
        main_cube_shader.set_bool("flashlight_on", flashlight_on);
        ...
        for (int i = 0; i < 10; i++) {
            ...
            main_cube_shader.set_mat4("model", new_model);
        ...
        light_cube_shader.set_vec3("light_color", glm::vec3(0.0f, 0.0f, 1.0f));

        //glDrawElements(GL_TRIANGLES, elems.size() * sizeof(unsigned int), GL_UNSIGNED_INT, 0);
        glDrawArrays(GL_TRIANGLES, 0, 36);

        for (int i = 0; i < 4; i++) {
            glm::mat4 new_model = glm::translate(glm::mat4(1.0f), pointLightPositions[i]);
            new_model = glm::scale(new_model, glm::vec3(0.2f));
            light_cube_shader.set_mat4("model", new_model);
            glDrawArrays(GL_TRIANGLES, 0, 36);
        }
        ...
        ImGui::Text("AYAYAYA");
        ImGui::Checkbox("Flashlight", &flashlight_on);
